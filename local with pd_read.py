    ...
    for files in dir_list:
        ...
        try:
            ...
            for sheet in sheet_list:
                ...
                sheet_no=sheet_no+1
                table_query="SELECT TARGET_TABLE FROM [GENERAL_SHEET] WHERE OBJECT_NAME ="+str(court+file_name+court)+" AND SHEET_NAME LIKE "+str(court+f_sheet_name+court)
                ...
                if str(table) !="[]":
                    ...
                    if str(target_table) != "None":
                        ...
                        column_index_query="SELECT COLUMN_INDEX FROM [GENERAL_SHEET] WHERE OBJECT_NAME ="+str(court+file_name+court)+" AND SHEET_NAME = "+str(court+f_sheet_name+court)+" AND TARGET_TABLE = "+str(court+target_table+court)
                        cursor.execute(column_index_query)
                        column_index=cursor.fetchone()
                        column_index=column_index[0]
                        data_index_query="SELECT DATA_INDEX FROM [GENERAL_SHEET] WHERE OBJECT_NAME ="+str(court+file_name+court)+" AND SHEET_NAME = "+str(court+f_sheet_name+court)+" AND TARGET_TABLE = "+str(court+target_table+court)
                        cursor.execute(data_index_query)
                        data_index=cursor.fetchone()
                        data_index=data_index[0]
                        type_index_query="SELECT TYPE FROM [GENERAL_SHEET] WHERE OBJECT_NAME ="+str(court+file_name+court)+" AND SHEET_NAME = "+str(court+f_sheet_name+court)+" AND TARGET_TABLE = "+str(court+target_table+court)
